...
class TransformerModel(nn.Module):
    ...
    def __init__(self, input_dim, n_head, sub_len, num_layer, n_embd,
                 forecast_history: int, dropout: float, scale_att, q_len, additional_params: Dict, horizon=None):
        ...
        if horizon:
            ...
            self.id_embed = nn.Embedding(horizon, n_embd)
    ...
    def forward(self, x: torch.Tensor):
        """
        Args:
            x: Required. Tensor of dimension (batch_size, seq_len, number_of_features)
        """
        batch_size = x.size(0)
...
class DecoderTransformer(nn.Module):
    def __init__(self, **param):
        """
        Args:
            current: Number of features present in input
            day: Number of features present in input
            seq_len: The number of historical steps fed into the time series model
            horizon: The number of targets to forecast
            n_embd: The dimention of Position embedding and time series ID embedding
            input_dim: Number of features present in input
            sub_len: sub_len of the sparse attention
            output_dim: Number of target to predict in output, only considering output_dim=1
            n_header: Number of heads in the MultiHeadAttention mechanism
        ...
        self.n_embd = int(param.get('n_embd'))
        self.input_dim = int(param.get('input_dim'))
        self.sub_len = int(param.get('sub_len'))
        ...
        self.additional_params = dict(param.get('additional_params'))
        ...
        self.transformer = TransformerModel(self.input_dim, self.n_header, self.sub_len, self.num_layers, self.n_embd, self.seq_len,
                                            self.dropout, self.scale_att, self.seq_len-self.sub_len , self.additional_params, horizon=self.horizon)
        self.sigma1 = nn.Conv1d(self.seq_len, self.horizon, kernel_size=self.input_dim+1)
        fc_dim = self.n_embd
        #transformer for day_sequence
        if self.day:
            self.day_variables = int(param.get('day_variables'))
            self.day_seq_len = int(param.get('day_seq_len'))
            self.day_transformer = TransformerModel(self.day_variables, self.n_header, 2 * self.sub_len, self.num_layers, self.n_embd, self.day_seq_len,
                                            self.dropout, self.scale_att, self.day_seq_len-self.sub_len, self.additional_params, horizon=self.horizon)
            self.sigma2 = nn.Conv1d(self.day_seq_len, self.horizon, kernel_size= self.day_variables + 1)
            fc_dim += self.n_embd
        # current
        if self.current:
            self.current_variables = int(param.get('current_variables'))
            self.emb_dim = int(param.get('emb_dim'))
            # self.encode_cur = MultiProSelectVariables(self.n_embd, self.current_variables, self.emb_dim,
            #                                               self.emb_dim * self.current_variables, 1)
            fc_dim += self.current_variables
    ...
    def forward(self, input: torch.Tensor, current=None, seq2_input=None):
        ...
        # transformer
        output = self.transformer(input)     # [bs, t, input_dim + n_embd]
        output = self.sigma1(output) # [bs,1, n_embd]

        if self.day:
            seq2_input = seq2_input[:, -self.day_seq_len:, -1, -self.day_variables:]
            d_output = self.day_transformer(seq2_input)   # [bs, t, day_variables + n_embd]
           # print('day',d_output.shape)
            d_output = self.sigma2(d_output)    # [bs,1, n_embd]
            output = torch.cat([d_output, output], dim=-1)  # [bs, 1,  2 * n_embd]
        output = output.squeeze(1)

        if self.current:
            _input = current[:, -self.current_variables:]
            output = torch.cat([output, _input], dim=-1)    # [bs, 2 * n_embd + current_variables]
