...
class cheb_conv_withSAt(nn.Module):
    def __init__(self, K, in_channels, out_channels, num_of_vertices):
    ...
    def forward(self, x, spatial_attention, chebpolynomial):
        '''
        ...
        for time_step in range(num_of_timesteps):
            ...
            output = torch.zeros(batch_size, num_of_vertices, self.out_channels).to(x.device)  # (b, N, F_out)
            for k in range(self.K):
                T_k = torch.Tensor(chebpolynomial[k]).to(x.device)  # (N,N)
...
class Temporal_Attention_layer(nn.Module):
    def __init__(self, in_channels, num_of_vertices, num_of_timesteps):
        ...
        self.time_emb = nn.Embedding(144, 64)
        self.U1 = nn.Parameter(torch.FloatTensor(num_of_vertices))
        self.U2 = nn.Parameter(torch.FloatTensor(in_channels + 64, num_of_vertices))
    ...
    def forward(self, x, time_id):
        _, num_of_vertices, num_of_features, num_of_timesteps = x.shape
        time_id = time_id.type(torch.int32)
        time_emb = self.time_emb(time_id[..., 1]) # (B, T, 64)
        lhs = torch.matmul(x.permute(0, 3, 2, 1), self.U1)  # x:(B, N, F_in, T) -> (B, T, F_in, N)(N) -> (B,T,F_in)
        lhs = torch.matmul(torch.cat((lhs, time_emb[:, -num_of_timesteps:, :]), dim=2), self.U2)
        # (B,T,F_in + 64)(F_in + 64,N)->(B,T,N)
...
class ASTGCN_block(nn.Module):
    def __init__(self, in_channels, K, nb_chev_filter, nb_time_filter, time_strides, num_of_vertices, num_of_timesteps):
        ...
        self.cheb_conv_SAt = cheb_conv_withSAt(K, in_channels, nb_chev_filter, num_of_vertices)
        self.time_conv = nn.Conv2d(nb_chev_filter, nb_time_filter, kernel_size=(1, 3), stride=(1, time_strides), padding=(0, 1))
        self.residual_conv = nn.Conv2d(in_channels, nb_time_filter, kernel_size=(1, 1), stride=(1, time_strides))

    def forward(self, x, time_id, chebpolynomial):
        '''
        :param x: (batch_size, N, F_in, T)
        ...
        # TAt
        temporal_At = self.TAt(x, time_id)  # (b, T, T)
        ...
        # cheb gcn
        spatial_gcn = self.cheb_conv_SAt(x, spatial_At, chebpolynomial)  # (b,N,F,T)
        ...
        x_residual = (x_residual + time_conv_output).permute(0, 2, 1, 3)
...
class ASTGCN_submodule(nn.Module):
    ...
    def forward(self, x, time_id, chebpolynomial):
        ...
        for block in self.BlockList:
            x = block(x, time_id, chebpolynomial)
...
class ASTGCN(nn.Module):
    def __init__(self, settings ):
        ...
        nb_block = 2
        self.K = 2
        nb_chev_filter, nb_time_filter = settings["hidden_dims"], settings["hidden_dims"]
        time_strides = 12
        self.horizon = settings["output_len"]
        len_input = settings["input_len"]
        self.num_node = settings["capacity"]

        self.submodule = ASTGCN_submodule( nb_block, self.input_dim - self.cat_num, self.K, nb_chev_filter, nb_time_filter, time_strides,
                                 self.horizon, len_input, self.num_node)
    ...
    def _reset_parameters(self):
        for p in self.parameters():
            if p.dim() > 1:
                ...
                nn.init.uniform_(p)
    def forward(self, x_enc, time_id, adj):
        """
        Args:
        ...
        """
        b = x_enc.shape[0]
        chebpolynomial = cheb_polynomial(adj, self.K)
        y = self.submodule(x_enc.permute(0, 1, 3, 2), time_id, chebpolynomial)
        y = y.reshape(b, self.num_node, self.horizon)
        return y
