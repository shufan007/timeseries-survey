...
class GruAttModel(nn.Module):
    ...
    def __init__(self, settings):
        ...
        self.t_emb_y = nn.Embedding(144, 143)
        self.w_emb_y = nn.Embedding(7, 6)
    ...
    def _reset_parameters(self):
        for p in self.parameters():
            if p.dim() > 1:
                ...
                nn.init.uniform_(p)
    def forward(self, x_enc, x_time_id, y_time_id):
        ...
        y_time_id = y_time_id.type(torch.int32)   # [b, t, 2]
        time_id = torch.cat( (self.t_emb(x_time_id[..., 1]), self.t_emb_y(y_time_id[..., 1])), dim=-1)
        weekday_id = torch.cat( (self.w_emb(x_time_id[..., 0]), self.w_emb_y(y_time_id[..., 0])), dim=-1)
        ...
        del x_att, x_enc, x_time_id, y_time_id
        # print('******* ', torch.cuda.memory_allocated() / 1024 / 1024)
