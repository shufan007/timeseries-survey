...
class SatGruAtt(nn.Module):
    ...
    def __init__(self, settings):
        ...
        self.gru = nn.GRU(input_size=(self.in_dim - self.cat_var),
                          ...
                          batch_first=True)
        self.spatio_att = Spatial_Attention_layer(self.in_dim - self.cat_var, self.nodes, self.seq_len)
    ...
    def forward(self, x_enc, x_time_id):
        ...
        satt = self.spatio_att(x_enc.permute(0, 1, 3, 2))
        ...
        x_gru_in = x_gru_in.permute(0, 1, 3, 2).reshape(batch_size, -1, self.nodes)
        x_gru_in = torch.matmul(x_gru_in, satt).reshape(batch_size, seq_len, invar, self.nodes)
        x_gru_in = x_gru_in.permute(0, 3, 1, 2).reshape(-1, seq_len, invar)
