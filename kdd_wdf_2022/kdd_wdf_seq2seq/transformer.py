...
class TransformerModel(nn.Module):
    ...
    def __init__(self, input_dim, n_head, sub_len, num_layer, n_embd,
        ...
        self.t_emb = nn.Embedding(144, 64)
        for i in [self.t_emb, self.w_emb]:
            nn.init.normal_(i.weight, std=0.02)
        self.n_embd = n_embd
        self.win_len = forecast_history

        self.fc = nn.Linear(64 + self.input_dim, self.input_dim)
        block = Block(n_head, forecast_history, self.input_dim, scale=scale_att, q_len=q_len, sub_len=sub_len)
    ...
    def forward(self, x: torch.Tensor, x_time_id):
        ...
        Returns:
        """
        embedding_x = self.t_emb(x_time_id[...,1])
        x = self.fc(torch.cat((x, embedding_x), dim=2))
...
class Transformer(nn.Module):
    def __init__(self, settings):
        ...
        self.sigma1 = nn.ModuleList([
            nn.Linear(self.input_dim * self.nodes, 256), nn.BatchNorm1d(256), nn.Tanh(),
            nn.Linear(256, 256), nn.BatchNorm1d(256), nn.Tanh(),
