...
def train_and_evaluate(config):
    ...
    if config["model_name"] == 'WPFModel':
        from orinal_model import WPFModel as network
    elif config["model_name"] == 'ASTGCN':
    ...
    for epoch in range(config["epoch"]):
        ...
        for id, (batch_x, batch_y) in enumerate(train_data_loader):
            ...
            with torch.cuda.amp.autocast():
                # np.savetxt("adj_matrix.csv", graph)
                pred_y = model((batch_x[..., 2:] - data_mean) / data_scale, batch_x[:, 0, :, :2], graph)
...
def evaluate(model,
    ...
    with torch.no_grad():
        for id, (batch_x, batch_y) in enumerate(valid_data_loader):
            ...
            step += 1
    del pred_y, batch_y, batch_x, valid_data_loader, graph
