...
from test_metrics import rmse, regressor_detailed_scores
...
def evaluate(settings):
    ...
    settings["path_to_test_x"] = os.path.join(settings["path_to_test_x"], './0001in.csv')
    settings["path_to_test_y"] = os.path.join(settings["path_to_test_y"], './0001out.csv')
    predictions = forecast_module.forecast(settings)
    test_y_ds = TestPGL4WPFDataset(filename=settings["path_to_test_y"], flag='out')
    grounds = np.array(test_y_ds.get_data()[0, :, -settings["output_len"]:, -1:])
    ...
    for idx in range(0, preds.shape[0]):
        acc = 1 - rmse(preds[idx, -day_len:], gts[idx, -day_len:]) / (settings["capacity"] * 1000)
        if acc != acc:
            continue
        day_acc.append(acc)
    ...
    overall_mae, overall_rmse, total_latest_mae, total_latest_rmse = regressor_detailed_scores(predictions, grounds, raw_data_lst, settings)
