...
class Autoformer(nn.Module):
    ...
    def __init__(self, settings):
        ...
        self.conv_block = nn.ModuleList([
            nn.Conv2d(self.input_dim - 2, self.d_model, kernel_size=(1, 30), dilation=(1, 2), padding=(0, 6)),
            nn.Conv2d(self.d_model, self.d_model * 2, kernel_size=(2, 12), padding=(1, 6), dilation=(2, 2)),
            nn.Conv2d(self.d_model * 2, self.d_model, kernel_size=(5, 6), padding=(0, 1), dilation=(2, 1))
        ])
    ...
    def forward(self, day_seq):
        ...
        day_seq = self.conv_block[0](day_seq) # [B, T_day, D]
        day_seq = self.conv_block[1](day_seq)
        day_seq = self.conv_block[2](day_seq).reshape(bs * nodes, self.d_model, -1)
        ...
        seasonal_init_day, trend_init_day = self.decomp(day_seq)   # [B, T_day, D]   ,  [B, T_day, D]
        ...
        seasonal_part_day, trend_part_day = self.decoder(seasonal_init, day_seq, trend=trend_init)# [B, T_out + T_day, D], [B, T_out + T_day, 1]
        del day_seq, trend_init, seasonal_init
