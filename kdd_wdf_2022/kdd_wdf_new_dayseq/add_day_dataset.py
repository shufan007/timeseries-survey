...
class PGL4WPFDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # s_begin = d_end - self.input_len
        # s_end = d_end
        r_begin = d_end
        r_end = r_begin + self.output_len
        day_x = self.build_day_seq(self.data_x[:, d_begin:d_end, 2:], self.day_seq)
        # seq_x = self.data_x[:, s_begin:s_end, :]
        seq_y = self.data_x[:, r_begin:r_end, :]

        if self.flag == "train":
            perm = np.arange(0, seq_y.shape[0])
            np.random.shuffle(perm)
            return day_x[perm], seq_y[perm]
        else:
            return day_x, seq_y
    ...
    def build_day_seq(self, day_data, day):
        ...
        for i in range(day):
            ...
            sbegin = 144 * (i + 1)
        new = np.stack(new, axis=1)
        return new

    def __len__(self):
        return self.data_x.shape[1] - self.day_seq * 144 - self.output_len + 1
...
class TestPGL4WPFDataset(Dataset):
    ...
    def __init__(self, filename, capacity=134, day_len=24 * 6, day_seq=14, flag='feat'):
    ...
    def data_preprocess(self, df_data):
        ...
        new_df_data.insert(0, 'weekday', weekday)
        pd.set_option('mode.chained_assignment', None)
        return new_df_data.replace(to_replace=np.nan, value=0), new_df_data
    ...
    def get_data(self):
        if self.flag == 'feat':
            day_data = self.build_day_seq(self.data_x[..., 2:], self.day_seq)
            return day_data
        else:
            return self.data_x
    def build_day_seq(self, day_data, day):
