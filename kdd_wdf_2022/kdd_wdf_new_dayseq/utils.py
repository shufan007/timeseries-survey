...
def save_model(output_path,
    ...
    torch.save(state,
               os.path.join(output_dir, "best_model_%s.pth" % steps ) )
...
def load_model(output_path, model, opt=None, lr_scheduler=None):
    ...
    if len(ckpt_paths) > 0:
        ...
        model_state_dict = torch.load(
            os.path.join(output_path, "best_model_10.pth"))
        for k, v in model_state_dict['model'].items():
            print(k)

        # model.load_state_dict({k.replace('module.submodule','submodule'):v for k,v in model_state_dict['model'].items()})
...
if __name__ == '__main__':
    ...
    from transformer import GruAttModel
