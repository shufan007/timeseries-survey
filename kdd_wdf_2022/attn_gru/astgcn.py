...
class cheb_conv_withSAt(nn.Module):
    def __init__(self, K, in_channels, out_channels, num_of_vertices):
        ...
        self.K = K
        self.cheb_polynomials = nn.ParameterList([nn.Parameter(torch.FloatTensor(num_of_vertices, num_of_vertices)) for _ in range(K)])
...
class Temporal_Attention_layer(nn.Module):
    ...
    def forward(self, x):
        ...
        # x:(B, N, F_in, T) -> (B, T, F_in, N)(N) -> (B,T,F_in)(F_in,N)->(B,T,N)
...
class ASTGCN(nn.Module):
    def __init__(self, settings ):
        ...
        nb_block = 2
        K = 2
        nb_chev_filter, nb_time_filter = 128, 128
        time_strides = 4
        self.horizon = settings["output_len"]
        len_input = settings["input_len"]
        self.num_node = settings["capacity"]

        self.submodule = ASTGCN_submodule( nb_block, self.input_dim - self.cat_num, K, nb_chev_filter, nb_time_filter, time_strides,
    ...
    def forward(self, x_enc, time_id):
        ...
        """
        b = x_enc.shape[0]
        y = self.submodule(x_enc.permute(0, 1, 3, 2))
