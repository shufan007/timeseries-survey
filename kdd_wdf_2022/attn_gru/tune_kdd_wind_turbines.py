...
Training
"""
def train_and_evaluate(config, config_env, isTune=True, distributed=True,
    ...
    if config_env["model_name"] == 'ASTGCN':
        from astgcn import ASTGCN as network
    elif config_env["model_name"] == 'GruAtt':
        from gru_att import GruAtt as network
    elif config_env["model_name"] == 'GruAtt_v2':
        #from gru_att import GruAtt as network
        from gru_att_v2 import GruAtt as network        
    elif config_env["model_name"] == 'Transformer':
        from transformer import Transformer as network
    
    config_env["num_layers"] = config["num_layers"]
    config_env["dropout_att"] = config["dropout_att"]
    config_env["dropout_proj"] = config["dropout_proj"]
    config_env["hidden_dims"] = int(2**config["hidden_dims"])
    config_env["epoch"] = config["epoch"]
                            
    model = network(config_env)
    model.to(device)
    ...
    cat_var_len = config_env["cat_var_len"]
    ...
    for epoch in range(int(round(config["epoch"]))):
        ...
        for id, (batch_x, batch_y) in enumerate(train_data_loader):
            ...
            if (id + epoch) % config_env["sample_step"]:
                continue
        ...
        if best_score == valid_r['score']:
            ...
            with distributed_checkpoint_dir(step=epoch) as checkpoint_dir:
                ...
                if dist.get_rank() == 0:
                    save_model(path, model.module, steps=epoch, opt=opt, lr_scheduler=lr_scheduler)
...
def automl_tune(args):
    ...
    config = {
        ...
        "dropout_att": tune.loguniform(1e-1, 3e-1),
        "dropout_proj": tune.loguniform(3e-1, 5e-1),
        "epoch": tune.randint(1, max_num_epoch),
        #"epoch": config_env['epoch'],
        #"batch_size": tune.randint(4, 6),  # log transformed with base 2
        "hidden_dims": tune.randint(5, 7),  # log transformed with base 2
    }
    ...
    result = flaml.tune.run(
        ...
        metric="score",
        mode="min",
        low_cost_partial_config={"epoch": 1},
    ...
    config_env["num_layers"] = int(best_trial.config["num_layers"])
    config_env["lr"] = best_trial.config["lr"]
    config_env["dropout_att"] = best_trial.config["dropout_att"]
    config_env["dropout_proj"] = best_trial.config["dropout_proj"]
    #config_env["batch_size"] = int(2**best_trial.config["batch_size"])
    config_env["hidden_dims"] = int(2**best_trial.config["hidden_dims"])
    config_env["epoch"] = best_trial.config["epoch"]
...
def task_arg_parser(argv, usage=None):
    ...
    parser.add_argument("--attention_mod", type=str, choices=['seq2v', 'self', 'both'], default='seq2v')
