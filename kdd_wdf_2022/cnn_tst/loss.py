...
__all__ = ["TweedieLoss", "SCORELoss", "SCORELoss2", "FilterMSELoss", "MSELoss", "HuberLoss", "MAELoss", "SmoothMSELoss"]
...
class TweedieLoss(nn.Module):
    ...
    def forward(self, x, y, raw, col_names):
        # x: predict
        # y: label
        x = torch.log(x)
...
class SCORELoss2(nn.Module):
    def __init__(self, **kwargs):
        super(SCORELoss2, self).__init__()

    def forward(self, pred, gold, raw, col_names):
        gold = gold.to(pred.device)
        
        examine_len = pred.shape[0]
        avg_mae, avg_rmse = turbine_scores(pred, gold, raw, examine_len, stride=1)
        
        score_loss = (avg_mae + avg_rmse)/2
        return score_loss.to(pred.device)    
...
class FilterMSELoss(nn.Module):
    ...
    def forward(self, pred, gold, raw, col_names):
        ...
        cond = cond.to(pred.device)
        gold = gold.to(pred.device)
        _mse = torch.mean(F.mse_loss(pred, gold, reduction='none') * cond)
        _mae = torch.mean(F.l1_loss(pred, gold, reduction='none') * cond)
